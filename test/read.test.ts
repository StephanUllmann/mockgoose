...
describe('Model Read Methods', () => {
  ...
  describe('Query.populate()', () => {
    it('should illustrate populated references if supported', async () => {
      const author = await MongooseAuthor.create(testAuthors[0]);
      ...
      expect(populatedMockgoose?.authors[0].name).toBe(
        populatedMongoose?.authors[0].name
      );
    });
  });
